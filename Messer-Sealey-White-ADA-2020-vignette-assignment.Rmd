...
```{r}
# Freedman TEST? <- this is for three groups so i think a wilcoxon comparing two paired groups may be better? 
...
In this new example, we showcase a selection of different aesthic changes you can utilie in your visiualization of network data based on monkey association patterns. 
```{r}
# first load in the new dataset
x <- read_csv("EM_monkey.csv", col_names=TRUE) 

# then remove the lower part of the matrix to delete the duplicate enteries. 
...
# next use the melt function to convert the matrix data into a dataframe 
...
# Concept Overview 
 ...
 Aesthic changes in visNetwork sociograms <add in brief code example here>
 Centrality statistics in igraph <code example here>
