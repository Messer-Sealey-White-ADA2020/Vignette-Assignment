...
# print iGraph using a force-directed layout (positions the nodes so that the edges have similar lengthes and there are as few crossing edges as possible)
...
## line color
tactiledata$nodes$label <- tactilenames
## node labels 
...
```{r}
# First we need to combine our centrality measures for both the aerial and tactical conditiosn into one dataframe, so 3 columns: id, treatment (aerial or tactile), centrality score and add in the name of the dataframe in the snake_wilcox analysis below and this should (hopefully) work...

#insert new dataframe here
######
######
######
# Ok the below is working to get the centrality measures into ne table:
Aname <- c(aerial$names)
Tname <- c(tactile$names)

Acent <- bind_cols(name = Aname, number = acentrality)
Acent
Tcent <- bind_cols(name = Tname, number = tcentrality)
Tcent
...
viper_centrality <- inner_join(Tcent, Acent, by = "name")
viper_centrality

viper_centrality <- viper_centrality %>%
  rename(
...
## now getting viper centrality to move from columns to stacked on top of each other is where I am stuck...



# Wilcoxon Test to compare the centrality scores for the snakes in the two different treatments 
snake_wilcox <- # dataname  %>& 
...
```{r}
# Read n another dataset, here we use monkeys 
